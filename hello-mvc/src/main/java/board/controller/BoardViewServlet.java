	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(!hasRead) {
				...
				// 쿠키 새로 전송 (boardCookie 가 없는 경우 | 요청된 boardCookie에 현재 no가 없는 경우)
				...
				response.addCookie(cookie); // 응답 헤더에 Set-Cookie로 전송
				System.out.println("> boardCookie가 새로 생성되었습니다.");
			}
			
			// 게시글 조회
			BoardExt board = boardService.findByNo(no);
			
			// XSS 공격 대비(Cross-site Scripting 공격)
			board.setTitle(board.getContent().replaceAll("<", "&lt;").replaceAll(">", "&gt;"));
