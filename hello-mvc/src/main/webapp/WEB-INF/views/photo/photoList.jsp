...
<section id="photo-wrapper">
	<h2>사진게시판 </h2>
	<% if(loginMember != null) { %>
	<input type="button" value="글쓰기" id="btn-add" onclick="location.href='<%= request.getContextPath() %>/photo/photoEnroll';" />
	<% } %>
		...
		success(resp) {
			...
			resp.forEach((photo) => {
				...
				img.onload = () => {
					...
					const div = `
					<div class="polaroid">
						<img src="\${img.src}" height="\${height}px" />
						<p class="info">
					...
					container.insertAdjacentHTML('beforeend', div); // 자식 요소로 맨 뒤에 추가	
				}
			});
		},
		error : console.log,
